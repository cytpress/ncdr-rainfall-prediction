...
# Load variables from environment (GitHub Secrets or .env)
...
def get_location():
    ...
    try:
        ...
        # ntfy might return multiple lines, take the last valid message
        for line in reversed(resp.split("\n")):
            data = json.loads(line)
            if data.get("event") == "message":
                # OwnTracks content is in message field
    ...
    try:
        ...
        if isinstance(grids, list):
            ...
            if data_list is None:
                print(f"API Error: Dict received but no data. Keys: {list(grids.keys())}")
